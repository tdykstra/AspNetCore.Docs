...
Non-sticky sessions in a web farm require a [#distributed-cache](xref:performance/caching/distributed) to avoid cache consistency problems. A distributed cache offloads the cache data from each server to an external process. For some apps, a distributed cache can support higher scale-out than an in-memory cache.
...
ASP.NET Core works with [SQL Server](https://www.microsoft.com/sql-server), [Redis](https://www.nuget.org/packages/Microsoft.Extensions.Caching.StackExchangeRedis), and [NCache](https://www.nuget.org/packages/Alachisoft.NCache.OpenSource.SDK/) distributed caches.

For more information, see <xref:performance/caching/distributed>.

## Hybrid cache

The `HybridCache` API bridges some gaps in the <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache> and <xref:Microsoft.Extensions.Caching.Memory.IMemoryCache> APIs. <xref:Microsoft.Extensions.Caching.Hybrid.HybridCache> is an abstract class with a default implementation that handles most aspects of saving to cache and retrieving from cache.

Hybrid cache has the following features that in-memory and distributed caches don't have:

* A unified API for both in-process and out-of-process caching.

  The `HybridCache` class is designed to be a drop-in replacement for existing `IDistributedCache` and `IMemoryCache` usage, but it also provides a simple API for adding new caching code. If the app has an `IDistributedCache` implementation, the `HybridCache` service uses it for secondary caching. This two-level caching strategy allows `HybridCache` to provide the speed of an in-memory cache and the durability of a distributed or persistent cache.
...
* Configurable serialization.
  ...
  Serialization is configured as part of registering the service, with support for type-specific and generalized serializers via the `WithSerializer` and `WithSerializerFactory` methods, chained from the `AddHybridCache` call. By default, the service handles `string` and `byte[]` types internally, and `System.Text.Json` for everything else. It can be configured for other types of serializers, such as protobuf or XML.
...
The equivalent code using the `HybridCache` class is simpler although the abstract class itself also provides features such as stampede protection:
...
The `HybridCache` library was released after .NET 9 was released, but it supports older .NET runtimes, including .NET Framework 4.7.2 and .NET Standard 2.0.
...
* By reducing the number of requests that clients and intermediate proxies make to the web server. This effect is achieved by setting HTTP headers such as `Cache-Control`, `Expires`, and `ETag`, which instruct clients and proxies on how to cache the responses. None of the other available types of caching have this capability.

Response caching has limited benefits for UI apps such as Blazor web apps because browsers generally set request headers that prevent caching. In contrast, [output caching](xref:performance/caching/output) can benefit UI apps by allowing server-side configuration to decide what should be cached.
...
* Enables caching of server responses based on [HTTP cache headers](https://developer.mozilla.org/docs/Web/HTTP/Headers/Cache-Control) like proxies do.
...
The output caching middleware enables caching of entire HTTP responses, like [response caching](#response-caching). Output caching differs from response caching in the following ways:

* The output caching behavior is configurable on the server.
...
* The output cache storage medium is extensible.
  ...
  Response caching is limited to memory.
